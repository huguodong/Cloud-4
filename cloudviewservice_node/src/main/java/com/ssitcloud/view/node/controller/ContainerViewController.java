...
public class ContainerViewController extends BasicController {
	...
	@ResponseBody
	public ModelAndView queryContainerByPage(HttpServletRequest request, String req) throws UnknownHostException {
		// return containerViewService.queryContainerByPage(req);
		String realPath = request.getServletContext().getRealPath("");//获取项目的绝对路径
		InetAddress inetAddress = InetAddress.getLocalHost();
		String address = inetAddress.getHostAddress();//获取本地ip地址
		Map<String, Object> model = new HashMap<>();
		model.put("oper", getCurrentUser());
		model.put("realPath", realPath);
		model.put("localAddress", address);
