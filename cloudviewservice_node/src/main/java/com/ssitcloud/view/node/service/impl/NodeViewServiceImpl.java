...
public class NodeViewServiceImpl extends BasicServiceImpl implements NodeViewService {
	...
	public ResultEntity queryAllNodes(String req){
		Map<String, String> map = new HashMap<>();
		
		String method = Thread.currentThread().getStackTrace()[1].getMethodName();
		String result = HttpClientUtil.doPost(requestURL.getRequestURL(url_prefix + method), map, charset);
		if(result != null){
			return JsonUtils.fromJson("",ResultEntity.class);
		}
	...
	public ResultEntity queryNodeGroupByName() {
		String method = Thread.currentThread().getStackTrace()[1].getMethodName();
		String result = HttpClientUtil.doPost(requestURL.getRequestURL(url_prefix + method), new HashMap<String,String>(), charset);
		if(result != null){
			return JsonUtils.fromJson(result, ResultEntity.class);
		}
		return new ResultEntity();
