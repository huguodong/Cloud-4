...
public class ChainDefinitionSectionMetaSource implements
	...
	public Section getObject() throws BeansException {
		...
		try {
			...
			try {
				 LogUtils.info("cloudviewservice_node  SelectMetaOperCmd");
				 LogUtils.info(cmd);
			} catch (Exception e) {
			}
			...
			if (cmd == null) {
				return section;
				//throw new CommonException("cloudviewservice_node ChainDefinitionSectionMetaSource.java:操作命令集获取异常 cmd IS NULL");
			}
			...
			if (!node.get("state").booleanValue()) {
				return section;
				//throw new CommonException("cloudviewservice_node ChainDefinitionSectionMetaSource.java:操作命令集获取异常  state is not true");
			}
			JsonNode resultNode = node.get("result");
			List<MetadataOpercmdEntity> cmds = JsonUtils.fromNode(resultNode,new TypeReference<List<MetadataOpercmdEntity>>() {});
			if (cmds == null) {
				throw new CommonException("cloudviewservice_node ChainDefinitionSectionMetaSource.java:操作命令集获转换异常 cmds is null");
			}
