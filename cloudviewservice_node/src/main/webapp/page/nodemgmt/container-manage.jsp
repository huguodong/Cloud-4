...
<div class="equipment-guanli">
        <div class="page-title-bar">
            ...
            <input type="hidden" value="${realPath}" id="realPath">
             <input type="hidden" value="${localAddress}" id="localAddress">
...
<script type="text/javascript">
	...
	$(function() {
		...
		$("tbody").on("click",".start",function(obj){
			...
			var save_path = $(this).parent().parent().find("input[name='save_path']").val();
			
			//判断当前的项目是否在启动的节点内
			//目标内网ip地址
			var host_ip1 = $(this).parent().parent().find("input[name='host_ip1']").val();
			//本地ip地址
			...
			var len = -1;
			if(realPath != null){
				len = realPath.indexOf(save_path);
			}
			if(host_ip1 == localAddress && len >=0){
				if(!confirm("【启动】操作将会重启当前项目，是否继续？")){
					return;
				}
			}
			var param={
				...
				if (data.state) {
					if (data.result.rows&& data.result.rows.length == 0) {
						...
						$.each(data.result.rows,function(i, item) {
							...
							+"<input type='hidden' name='host_ip1' value='"+item.host_ip1+"' /><input type='hidden' name='save_path' value='"+item.save_path+"' />"
							+ item.container_id
							+ "</td>"
							+ "<td class='col3'>"
							+ item.container_name
