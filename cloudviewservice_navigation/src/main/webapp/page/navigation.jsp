...
<body>
    <div class="page-top"><div id="lmsg">${lib_name}${locations[1]}楼${locations[2]}区${locations[3]}排${locations[4]}面${locations[5]}架${navigationInfoEty.shelf_layer}层</div></div>
   	<input type="hidden" value="${locations[3]}排${locations[4]}面${locations[5]}架架${navigationInfoEty.shelf_layer}层" id="locationMsgInput"/>
    <div class="page-down">
...
<script>
    window.onload = function(){
    	...
    	if(shelfImageUrl.length > 0){
    		//$("#shujiaCanvas").css("background-image", 'url("${pageContext.request.contextPath}/static/images/' + shelfImageUrl + '")')
    		$("#shujiaCanvas").css("background-image", 'url(/temp/'+libId+'/' + shelfImageUrl + ')')
    	}
        //标题闪动
        var i=0,add=1;
        function FadeText()
    	...
    	if(floorImageUrl.length > 0){
    		...
    		img.src = '/temp/'+libId+'/'+floorImageUrl;
    	}else{
            ...
            function TM(){
                setInterval(function(){
                    if ( i < nos ) {
                        ...
                        dest_y = parseFloat(parseInt(document.getElementById("route_yy" + String(i+1)).innerHTML)*line_factor_y);
