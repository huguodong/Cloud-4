...
import com.ssitcloud.app_operator.presenter.RegionPresenter;
import com.ssitcloud.app_operator.view.viewInterface.RegionViewI;

import java.util.List;


public class RegionActivity extends Activity implements RegionViewI {
...
//            Toast.makeText(RegionActivity.this,tag.getRegi_idx().toString(),Toast.LENGTH_SHORT).show();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        List<LibraryEntity> list = (List<LibraryEntity>) bundle.getSerializable("libs");
        ...
        View back = findViewById(R.id.region_return);
        back.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent i = new Intent();
                i.putExtra("region",new RegionEntity());
                setResult(RESULT_OK,i);
                RegionActivity.this.finish();
            }
        });

        String operatorType = app.getOperator_type();

        if (OperatorEntity.SSITCLOUD_ADMIN.equals(operatorType) || OperatorEntity.SSITCLOUD_MANAGER.equals(operatorType)){
            presenter.loadData(null,"01");
        }else{
            presenter.loadDataForNormal(list);
        }
    }
    ...
    public void setProvince(List<RegionEntity> provinces) {
        ...
        if(provinces.isEmpty()){
            findViewById(R.id.provinceSV).setVisibility(View.GONE);
            return ;
        }
