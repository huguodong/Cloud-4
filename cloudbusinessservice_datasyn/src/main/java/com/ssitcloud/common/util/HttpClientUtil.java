...
import com.ssitcloud.common.exception.HttpResponseException;
import com.ssitcloud.common.exception.HttpTimeoutException;
/**
...
//    private static final String CONTENT_TYPE_TEXT_JSON = "text/json";
    ...
    static{
		try {
			config=PropertiesLoaderUtils.loadProperties(new EncodedResource(new ClassPathResource("config/static-config.properties"), Consts.UTF_8.toString()));
	...
	public static String doPost(String url,Map<String,String> map,String charset){
		Log.DebugOnScr("Datasync http doPost url:"+url);
		Log.DebugOnScr("Datasync http doPost map:"+map);
		...
		try{
			...
			//start author by LXP 增强异常逻辑，精准定位异常
	...
	public static String doPostLongtime(String url,Map<String,String> map,String charset){
		Log.DebugOnScr("Datasync http doPostLongtime url:"+url);
		Log.DebugOnScr("Datasync http doPostLongtime map:"+map);
		HttpPost httpPost = null;
		String result = null;
		CloseableHttpClient httpClient = null;
		CloseableHttpResponse response=null;
	 ...
	 * @return
	 */
	public static String postUpload(String url,File file,String libId,String deviceId){
		String result = null;
		try{
	        ...
	        String encoderJson2 = URLEncoder.encode(deviceId, Consts.UTF_8.toString());
			 // 相当于<input type="file" name="file"/>
			HttpEntity reqEntity = MultipartEntityBuilder.create().setCharset(Consts.UTF_8)
					.setMode(HttpMultipartMode.BROWSER_COMPATIBLE).addPart("file", bin).addTextBody("libId", encoderJson).addTextBody("deviceId", encoderJson2)
					.build();
