...
public class DeviceMonConfController extends BasicController{
	...
	public ResultEntity AddMonitorConfTemp(HttpServletRequest request,String req){
		...
		try {
			...
			SystemLogUtil.LogOperation(result, getCurrentUser(), request, Const.OPERCMD_ADD_DEVICE_MONITOR_TEMP);
		} catch (Exception e) {
			ExceptionHelper.afterException(result,Thread.currentThread().getStackTrace()[1].getMethodName(),e);
		}
		return result;
	...
	/**
	 * 注意 只适用于 更新模板，不适用与自定义数据<br>
	 * 
	 * 更新 监控配置模板 /更新操作是要先删除所有 再新增
	 * dbservice   DeviceMonitorConfController.java -->函数  UpdDeviceMonitorConfAndTemp
	 * <br>
	 * 格式
	 * req={
	 * 		 DeviceMonTempEntity device_mon_tpl;  //device_mon_tpl_idx
			 List<DeviceMonConfigEntity> dev_mon_conf_Arr;
	 * }<br>
	 * <br>
	 * @param request
	 * @param req
	...
	public ResultEntity UpdMonitorConfTemp(HttpServletRequest request,String req){
		...
		try {
			...
			SystemLogUtil.LogOperation(result, getCurrentUser(), request, Const.OPERCMD_UPDATE_DEVICE_MONITOR_TEMP);
		} catch (Exception e) {
			ExceptionHelper.afterException(result,Thread.currentThread().getStackTrace()[1].getMethodName(),e);
		}
		return result;
	...
	public ResultEntity DelMonitorConfTemp(HttpServletRequest request,String req){
		...
		try {
			...
			SystemLogUtil.LogOperation(result, getCurrentUser(), request, Const.OPERCMD_DELETE_DEVICE_MONITOR_TEMP);
		} catch (Exception e) {
			ExceptionHelper.afterException(result,Thread.currentThread().getStackTrace()[1].getMethodName(),e);
		}
		return result;
