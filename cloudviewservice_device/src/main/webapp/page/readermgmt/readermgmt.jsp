...
<div class="equipment-leixing">
	<div class="page-title-bar">
		<span class="title">读者模板管理</span>
		<div class="form-wrap fr">
			<div class="g-select">
					<select id="statType">
						<option value="-1" selected>---选择类型---</option>
						<option value="1" >读者导入配置模板</option>
			...
			<div class="btn increase increase-layer">
				...
				<div class="increase-list">
					<div class="icon"></div>
					<ul>
						<li><a href="javascript:template(1);"><strong>+</strong> 读者导入配置模板</a></li>
...
$(function(){
	...
	**/
	$(".form-btn.remove").on("click",function(){
		...
		$.ajax({
			...
			if(data.state){
				...
				var Page={"page":currentpage,"pageSize":size,"import":statType,"import_tpl_type":"1","import_tpl_desc":str,"type":"reader","lib_id":lib_id};
				//调用页面的查询ajax
				$.select(Page);  
		     }else if(data.retMessage){
		     	showRetMessage(data.retMessage);
	...
	$(document).ready(function(){
		...
		var Page={"page":startpage,"pageSize":size,"import_tpl_type":"1","import_tpl_desc":"","type":"reader","lib_id":lib_id};
		$.ajax({
	...
	$("#page").on("click",".prev-page",function(){
		...
		var size= $('#pagesize option:selected').text();
		var str=$("#Keyword_lib_idx").val();
		var Page={"page":page,"pageSize":size,"import_tpl_type":"1","import_tpl_desc":str,"type":"reader","lib_id":lib_id};
		$.select(Page);
	});
	//下一页操作
	...
	**/
	$(".search").on("click",function(){
		...
		var Page;
		console.info("statType:",statType);
		var str=$("#Keyword_lib_idx").val();
		var size= $('#pagesize option:selected').text();
		if((statType==null||statType==undefined||statType=="")&&(str==null||str==undefined||str=="")){
			Page={"page":"1","pageSize":size,"import_tpl_type":"1","import_tpl_desc":str,"type":"reader","lib_id":lib_id};
		}else{
	...
	**/
	$("select#pagesize").on("change",function(){
		...
		var size= $('#pagesize option:selected').text();
		var str=$("#Keyword_lib_idx").val();
		var Page={"page":1,"pageSize":size,"import_tpl_type":"1","import_tpl_desc":str,"type":"reader","lib_id":lib_id};
		$.select(Page);
	});
});
	var bakUpload={};
		...
		$.ajax({
			...
			success:function(data){
				...
				var currentpage = $(".paging-bar li.active").text();
		     	var size= $('#pagesize option:selected').text();
		     	var str=$("#Keyword_lib_idx").val();
				var Page={"page":currentpage,"pageSize":size,"import_tpl_type":"1","import_tpl_desc":str,"type":"reader"};
