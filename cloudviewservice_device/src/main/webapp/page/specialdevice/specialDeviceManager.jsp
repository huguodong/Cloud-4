...
<div class="equipment-guanli">
	<div class="page-title-bar">
		...
		<span class="title"><sp:message code="设备管理"></sp:message><a href="${pageContext.request.contextPath}/help/main?url=/page/common/help/devmgmt/devicemgmt.jsp" target="_blank" class="g-help"></a></span>
		<div class="form-wrap fr">
			...
			<input type="text" name="" id="device_name" class="input g-input" placeholder="<sp:message code="请输入设备名称"></sp:message>" />
			...
			<div class="btn increase g-btn-green" ><sp:message code="添加设备"></sp:message></div>
		...
		<table class="g-table">
			<thead>
				<tr>
					...
					<th class="col2"><sp:message code="设备类型"></sp:message></th>
					<th class="col3"><sp:message code="设备ID"></sp:message></th>
					<th class="col4"><sp:message code="设备名称"></sp:message></th>
...
$(function(){
	...
	$("tbody").on("click",".delete",function(){
		idx.length = 0;
		idx.push( $(this).parent().parent().find("div input[name='idx']").attr("id"));
		$('.g-delete-dialog .word').html("");
	...
	$("tbody").on("click",".edit",function(){
		...
		$("#mainframe").load(basePath+"/specialDevice/editSpecialDevice",{"req":JSON.stringify(param),"returnInfo":JSON.stringify(infoParam)},function(){
	...
	$(".form-btn.remove").on("click",function(){
		...
		$.ajax({
			url:basePath+"/specialDevice/deleteSpecialDevice",
			type:"POST",
			data:{
	...
	jQuery.select=function(param){
		...
		$("tbody").html("");
		$.ajax({
			url:basePath+"/specialDevice/querySpecialDeviceByPage",
			type:"POST",
			...
			success:function(data){
				...
				$.each(data.result.rows,function(i,item){
					...
					$("tbody").append( "<tr>"+
			            "<td class='col1'><div class='g-checkbox'><input type='checkbox' name='idx' id='"+item.idx+"' /></div></td>"+
						"<td class='col2'>"+item.device_type_desc+"</td>"+
						"<td class='col3'>"+item.device_id+"</td>"+
						"<td class='col4 device_id'>"+item.device_name+"</td>"+
						"<td  class='col11' id='"+item.idx+"_lib' style='display:none'>"+item.library_idx+"</td>"+
					...
					$.each(libraryInfos,function(i,libInfo){
						...
						if(libInfo.library_idx == item.library_idx){
							...
							$("#"+item.idx+"_lib").html(libInfo.lib_name);
						}
				      }); 
					 $("#"+item.idx+"_lib").show();
				});
		...
		$.ajax({
			...
			$("#type").html("<option value='-1' selected>"+devicemgmt_jsp_selectdevtype+"</option>");
				$.each(data.result,function(i,item){
					$("#type").append(
					"<option value='"+item.device_type_id+"'>"+item.device_type_desc+"</option>" 
					);
	            });
	...
	**/
	...
	$(function() {
		...
		var returnInfo = {};
		returnInfo.page = 1;
		returnInfo.pageSize = $('#showSize option:selected').text();
		//返回按钮参数
		var specialDevice=<%=request.getAttribute("specialDevice")%>;
		if(specialDevice != null){
			
			returnInfo.library_idx = specialDevice.library_idx;
			returnInfo.device_type_id = specialDevice.device_type_id;
			returnInfo.device_name = specialDevice.device_name;
			returnInfo.page = specialDevice.page;
			returnInfo.pageSize = specialDevice.pageSize;
			
			$("#type").val(returnInfo.device_type_id);
			
			$("#device_name").val(returnInfo.device_name);
	...
	$(".increase").on("click",function(){
		...
		$("#mainframe").load(basePath+"/specialDevice/addSpecialDevice",{"req":JSON.stringify(param)},function(){
			$(".form-dialog-fix .form-dialog").each(function(){
				$(this).attr("date-right",$(this).css("right"));
			});
		});
