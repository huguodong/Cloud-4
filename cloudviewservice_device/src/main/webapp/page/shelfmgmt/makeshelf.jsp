...
<div class="form-dialog" id="shelfMark">
	...
	<div class="form-wrap">
		<span style="padding-left: 30px;">******** 制作过程可能出现错误异常，中途出现异常请先删除制作产生的数据 ***********</span>
		<ul>
			<li><!--红色框体 li  class="error" -->
				<div class="left"><span class="g-mustIn">图书馆ID</span></div>
				<div class="right">
					<input type="text" name="" id="markshelf_libId" class="g-input" placeholder="<spring:message code="message.gloable.pleaseinput"/>"  />
				</div>
			</li>
			<li>
				<div class="left"><span class="g-mustIn">馆名称</span></div>
				<div class="right">
					<input type="text" name="" id="markshelf_libName" class="g-input" placeholder="<spring:message code="message.gloable.pleaseinput"/>" readonly="readonly"  style="color:gray;background-color:#E0E0E0"/>
				</div>
			</li>
			<li><!--红色框体 li  class="error" -->
...
$(function(){
	...
	**/
		...
		$.ajax({
			...
			success:function(data){
				if(data.state){
					...
					var Json = {"markshelf_libId":lib_id,"markshelf_zoneName":markshelf_zoneName}
	...
	$("#markShelfBtn").on("click",function(){
		...
		if(oper.operator_type>=3){
			$('#shelfMark').find("#markshelf_libId").prop("readonly","readonly").prop("style","color:gray;background-color:#E0E0E0");
			$('#shelfMark').find("#markshelf_libId").val(libIdxAndNameObj[oper.library_idx].lib_id);
			$('#shelfMark').find("#markshelf_libName").val(libIdxAndNameObj[oper.library_idx].lib_name);
		}
	...
	$("#makeStart").on("click",function(){
		...
		var markshelf_libId=$("#markshelf_libId").val();
		...
		param = {
				"markshelf_libId":markshelf_libId,
	...
	**/
        ...
        if(obj)
    {
       ...
       return obj.value;
		    ...
		    $.ajax({
				...
				if(data){
					...
					if(oper.operator_type>=3){
						...
						var Page = {"page":"1",
								"pageSize":size};
						var Json = {"markshelf_libId":lib_id}
						var param={	json: Json , page : Page};
						$.ajax({
						url:basePath+"/makeshelf/queryAllMakeShelfRecord",
						type:"POST",
		 ...
		 *	监听图书馆ID变化 改变对应的图书馆名
		 **/
		$("#markshelf_libId").on("keyup",function(){
			var lib_id=$(this).val();
			if(lib_id&&libIdAndNameObj[lib_id]){
				$("#markshelf_libName").val(libIdAndNameObj[lib_id].lib_name);
			}else{
				$("#markshelf_libName").val("");
			}
		});
