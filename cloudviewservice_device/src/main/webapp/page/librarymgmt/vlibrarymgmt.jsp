...
<div class="library-guanli">
		<div class="page-title-bar">
			<span class="title">虚拟馆管理<a href="${pageContext.request.contextPath}/help/main?url=/page/common/help/librarymgmt/librarymgmt.jsp" target="_blank" class="g-help"></a></span>
		...
		<div class="tac-box">
			<div class="con-area">
				...
				<!--去除上面的full-line类名就是虚线啦 -->
					<ul>
						<li><div class="g-guan">深圳市虚拟馆</div></li>
					</ul>
				</div>
				...
				<!--去除上面的full-line类名就是虚线啦 -->
					<div class="item"><div class="g-guan">深圳市虚拟馆</div></div>
			...
			<div class="con-area" >
				...
				<!--去除上面的full-line类名就是虚线啦 -->
					<ul>
						<li><div class="g-guan theme-green">深圳市虚拟馆</div></li>
					</ul>
				</div>
...
<div class="form-dialog fenzu-dialog" id="fenzu-dialog">
	...
	<div class="form-wrap">
		<ul>
			<li>
				<div class="left"><span class="g-mustIn">虚拟馆ID</span></div>
				<div class="right">
            ...
            <li>
                <div class="left"><span class="g-mustIn">虚拟馆名称</span></div>
                <div class="right">
            ...
            <li>
                <div class="left"><span class="g-mustIn">虚拟馆地址</span></div>
			...
			<li>
				...
				<div class="right">
					 ...
					 <!-- 只有云平台管理人员和维护人员可以修改虚拟馆服务期限	modify by huanghuang 20170221 -->
					 <%-- <shiro:hasPermission name="0102010106"> --%>
					 <c:if test="${operator!=null and (operator.operator_type==1 or operator.operator_type==2) }">
						 <input type="button" value="设置虚拟子馆" class="edit-lib-rel g-btn-blue"/>
					 </c:if>
					 <%-- </shiro:hasPermission> --%>
				</div>
			</li>
...
var tmpIdx = -1;//虚拟馆的模板暂存字段,新增虚拟馆时，会用到。add by huanghuang 20170221
var libPosLayer;
$(function(){
	...
	$(".delete").on("click",function(){
		...
		var num = $("#maintable tbody input[name='idx']:checked").length;
		$("#maintable tbody input[name='idx']:checked").each(function(){
		...
		if(num>0){
			...
			$(".g-delete-dialog .word").append("当前选择了<span class='font-red'> "+num+" </span>条数据<br>是否要删除选择的虚拟馆信息？");
			delLayer = layer.open({
				type: 1,
				shade: false,
				title: false, //不显示标题
	...
	function GetDataToLayer(_this){
	    ...
	    //修改虚拟馆服务模板权限 by huanghuang 20170215
	...
	$("#maintable tbody").on("click",".config",function(){
	...
	$("#maintable tbody").on("click",".edit",function(){
		...
		GetDataToLayer(this);
		operationFlag="edit";
		var relLibIdxStr = $(this).parents("tr").find(".relLibIdxStr").html();
		$("#otherlist").val(relLibIdxStr);
	...
	$("#fenzu-dialog .submit").on("click",function(){
		...
		if(otherlist!=""){
			...
			for(var i=0; i<exts.length; i++){
				var rel_lib = {};
				rel_lib.slave_lib_id = exts[i];
				rel_lib.rel_type = "1";  <%-- 1:主从关系   --%>
				relSubLibs.push(rel_lib);
			}
		}
		lib_param.relSubLibs = relSubLibs;
	...
	**/
	...
	$("#maintable tbody").on("click",".relation",function(){
	...
	jQuery.select= function(param){
		...
		$.ajax({
						...
						if(data.result.rows&&data.result.rows.length==0){
							...
							var relLibIdxStr = "";
							 if(item.relLibs.length !=0){
								 $.each(item.relLibs,function(k,relLib){
									relLibIdxStr += relLib.slave_lib_id + ",";
								 });
								 relLibIdxStr = relLibIdxStr.substring(0, relLibIdxStr.length -1);
								...
								"<td class='relLibIdxStr hide'>" + relLibIdxStr+ "</td>"+
								"</tr>");
							}else{
								var service_expire_date=item.service_expire_date;
								var service_start_date=item.service_start_date;
	    	...
	    	//查看虚拟馆关系权限判断，add by huanghuang 20170216
			...
			success:function(data){
				...
				$.each(data.result.rows,function(i,item){
					...
					if(item.relLibs.length !=0){
						...
						"<td class='col3'>" + address + "</td>"+
						"<td class='col4'>" + phone + "</td>"+
						"<td class='col5'>" + service_expire_date + "</td>"+
						"<td class='col6'>" + service_start_date+ "</td>"+
						"<td class='col7'>" +(item.lib_service_tpl_id==null?'否':'是('+item.lib_service_tpl_id+')') + "</td>"+
						"<td class='col8'>"+
							"<span class='btn-a edit'>编辑</span>"+
							"<span class='btn-a config g-btn-blue'>配置</span>"+
								...
								$.ajax({
									...
									if(data&&data.state==true){
										...
										if(msg.indexOf("optimistic")>=0){
											layer.alert("当前虚拟馆模板数据已经被其他人修改，请刷新之后操作");
										}
									}
								}).then(function(){
	...
	**/
	...
	function qyeryDeviceExtList(){
		var param={"lib_type":0};
		$.ajax({
			url:"${pageContext.request.contextPath}/library/selActualLibraryMaster",
			type:"POST",
			data:{"req":JSON.stringify(param)}
		}).done(function(data){
			...
			if(data && data.state){
				...
				//console.log("actualMasterLib",result);
				if(result){
						var html = "";
						for(var i=0; i<result.length; i++){
							var item = result[i]
							html += "<tr>";
							html += "<td class=\"col1\"><div class=\"g-checkbox\" ><input type=\"checkbox\" name=\"\" value=\""+item.library_idx+"\" /></div></td>";
							html += "<td class=\"col2\">"+item.lib_id+"</td>";
							html += "<td class=\"col3\">"+item.lib_name+"</td>";
							html += "</tr>";
						}
	...
	$(".edit-lib-rel.g-btn-blue").on("click",function(){
		<%-- 先把所有的取消选择 --%>
