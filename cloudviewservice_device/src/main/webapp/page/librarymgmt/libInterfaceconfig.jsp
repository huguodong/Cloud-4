...
<div class="page-title-bar">
	...
	<span class="title">图书馆级配置<a href="#" class="g-help"></a></span>
</div>
...
$(function(){
			...
			returnInfo.lib_type = id.lib_type;
			
			returnInfo.library_idx=id.library_idx;//id.library_idx;
			...
			$(".main").css({
			 ...
			 cfghtml += '<div id="lib_div" class="item"><ul><li>';
			 cfghtml += '<div class="left"><s:message code="deviceedit.jsp.libname"/></div>';
			 cfghtml += '<div class="right">';
			 cfghtml += '<div class="g-select"><select id="lib"></select>';
		     ...
		     var lib_type = id.lib_type;
		     var libid = id.library_idx;/* '167'; oper.lib_id; */
		     //alert("operator_type"+operator_type+"  libid"+libid);
		     if(operator_type=="3"||operator_type=="0"||operator_type=="1"){
					...
					$.ajax({
					 	...
					 	success:function(data){
							...
							console.log("lib",LibInfo);
							
					 		//查询后获得设备所属的library_idx、device_name、device_model_idx
						   	library_idx = LibInfo[0].library_idx;
						   	var lib_name = LibInfo[0].lib_name;
						   	var lib_id = LibInfo[0].lib_id;
						   	
					 		//将deice的所有option项先删除
					 		$("#lib").find("option").remove();
					 		//显示设备显示的div
							$("#lib_div").show();
							$("select[id='lib']").css("display","block");	
						   	
												 			
					 		$("#lib").find("option").remove();
					 			
					 		op = new Option(lib_name+"("+lib_id+")",library_idx);
					 		deviceSel = document.getElementById("lib").add(op);
					 		//alert("-----------------"+library_idx);
							//显示图书馆配置界面	
							showInterface(library_idx,null);
					 	}
					});
			}
		    ...
		    $("a.back-btn").on("click",function(){
		 		if(returnInfo){ 
		 			if(lib_type == 1){
		 				$("#mainframe").load('<%=basePath%>/library/vmain',{"returnInfo":JSON.stringify(returnInfo)},function(){});
		 			}else{
		 				$("#mainframe").load('<%=basePath%>/library/main',{"returnInfo":JSON.stringify(returnInfo)},function(){});
		 			}
		 		} 
		 	});
});
function showInterface(library_idx,deviceId){
	var data1;
	var html = '';
	
	library_idx_d = library_idx;
	device_idx_d = deviceId;
		 		...
		 		success:function(data){
		 			...
		 			if(data1!=null){
			 			...
			 			if(data1[0]!=null){
			 				...
			 				if(data1[0]==null){
			 					//如果设备未
			 					$("select[id!='lib']  option[value='-1']").prop("selected",true);
			 					$("select[id!='lib']").next("input").css("display","none");
			 				}
			 			}
		 	 		}
		 		},
...
function submit(){
	...
	if(!diy_none){
		...
		if(flag){
			...
			jsonStr = '[';
			$("#mainDiv div select[id!='lib']").each(function(index){   
		 		...
		 		success:function(data){
		 			...
		 			if(!importState){
						   $.ajaxFileUpload({
								 ...
								 type:'post',
								 data: { libId: lib_id, deviceId: device_idx_d},
								 dataType:'json',
