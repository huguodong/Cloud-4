...
public class HttpClientUtil {
    ...
    private static int _POST_REGISTER_CONNECTION_TIMEOUT;
    private static int _POST_REGISTER_SOCKET_TIMEOUT;
    private static RequestConfig reqConfig;
    private static RequestConfig registerConfig;
    ...
    static{
		try {
			...
			String post_register_socket_timeout = config.getProperty("post_register_socket_timeout", "60000");
			String post_register_connection_timeout = config.getProperty("post_register_connection_timeout", "60000");
			...
			_POST_REGISTER_SOCKET_TIMEOUT = Integer.valueOf(post_register_socket_timeout);
			_POST_REGISTER_CONNECTION_TIMEOUT = Integer.valueOf(post_register_connection_timeout);
			reqConfig =RequestConfig.custom().setSocketTimeout(_post_socket_time_out).setConnectTimeout(_post_connection_time_out).build();
			registerConfig = RequestConfig.custom().setSocketTimeout(_POST_REGISTER_SOCKET_TIMEOUT).setConnectTimeout(_POST_REGISTER_CONNECTION_TIMEOUT).build();
	 ...
	 * @return
	 */
	public static String doPostForRegister(String url,Map<String,String> map,String charset){
		HttpPost httpPost = null;
		String result = null;
		try{
			httpPost = new HttpPost(url);
			//设置请求和传输超时时间
			httpPost.setConfig(registerConfig);
			//设置参数
			List<NameValuePair> list = new ArrayList<NameValuePair>();
	...
	public static String doPostLongTimeout(String url,Map<String,String> map,String charset){
					...
					if(response != null){
						...
						if(code>300){
							if(code==HttpStatus.SC_NOT_FOUND){//
								ResultEntity newRes=new ResultEntity();
								newRes.setRetMessage("请求"+url+"--->404 访问不到服务");
								LogUtils.error(newRes.getRetMessage());
								result=JsonUtils.toJson(newRes);
							}else if(code==HttpStatus.SC_INTERNAL_SERVER_ERROR){
								ResultEntity newRes=new ResultEntity();
								newRes.setRetMessage("请求"+url+"--->500 访问服务异常");
	 ...
	 public static final int cache = 1024*1024;//M
	 public static void main(String[] args) {
		
	}
	/** 
     * 根据url下载文件，保存到备份文件路径+/view中 
     * @param url  本地路径
     * @param filepath  已经包括 fileName
     * @return 
     */  
    public static File downloadBakFile(String url,String filepath,String fileName,Map<String,String> map) {  
        try {
	 ...
	 * @return
	 */
	public static String postUploadLibDev(String url,File file,String libId,String deviceId){
		String result = null;
		try{
	        ...
	        String encoderJson1 = URLEncoder.encode(deviceId, Consts.UTF_8.toString());
			 // 相当于<input type="file" name="file"/>
			HttpEntity reqEntity = MultipartEntityBuilder.create().setCharset(Consts.UTF_8)
					.setMode(HttpMultipartMode.BROWSER_COMPATIBLE).addPart("file", bin).addTextBody("libId", encoderJson).addTextBody("deviceId", encoderJson1)
					.build();
