...
<!-- 		<a class="exit" href="<%=basePath %>device/connect?deviceCode=${deviceCode }"><span class="icon"></span>退出</a> -->
		<a class="exit" href="<%=basePath %>login/logout"><span class="icon"></span>退出</a>
...
// 		$(".shade").eq(1).trigger("click");
	...
	$(".g-submit").on("click",function(){
		...
		<%-- 数据提交，设备注册 --%>
		$.ajax({
			url:"<%=basePath%>hydevice/hydeviceRegister",
			type:"POST",
			data:param,
			success:function(data){
				if(data!=null ){
