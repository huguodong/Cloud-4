...
import com.ssitcloud.view.statistics.common.util.PropertiesUtil;
...
public class AppConfigController {
	...
	public ResultEntity clearAppConfig(HttpServletRequest request, String req) {
		...
		if ("true".equals(developer_model)) {
			baseUrl = PropertiesUtil.getConfigPropValueAsText("cloudbusinessservice_node_url_developer");
		} else {
			baseUrl = PropertiesUtil.getConfigPropValueAsText("cloudbusinessservice_node_url");
		}
		ResultEntity resultEntity = appConfigService.clearAppConfig(request, requestURL, req, baseUrl + node_interface);
		// servlet重新加载bean
