			...
			if(success){
				...
				if(oper!=null){
					...
					if(Operator.SSITCLOUD_ADMIN.equals(oper.getOperator_type())){
						...
						//获取所有菜单
						menu=userService.SelMenuBySsitCloudAdmin(menuType);
						
					}else{
						//获取权限
					    res=userService.SelPermissionByOperIdx(oper.getOperator_idx());
					    //根据权限获取菜单
					    menu=userService.SelMenuByOperIdx(oper.getOperator_idx(),menuType);
					}
					...
					if(JSONUtils.mayBeJSON(menu)){
						JsonNode node=JsonUtils.readTree(menu);
						if(node!=null){
							if(node.get("state").booleanValue()){
								List<UserMenuPermessionEntity> menuPermessions=JsonUtils.fromNode(node.get("result"),new TypeReference<List<UserMenuPermessionEntity>>() {});
								if(menuPermessions!=null){
									oper.setUserMenuPermessions(menuPermessions);
								}
							}
						}
					}
