...
@Component(value="metadata_ext_model")
public class DownMetadataExtModel implements TableCommand{
		
	@Resource
	private MetadataExtModelDao extModelDao;
		...
		try {
			...
			if(devconfs!=null&&devconfs.size()>0){
				...
				if(DeviceConfigEntity.IS_MODEL.equals(ext_tpl_flg)){
						...
						List<MetadataExtModelRemoteEntity> metadataExtModels=extModelDao.selectListByDeviceExtConfigDown(deviceExtConfig);
						if(metadataExtModels!=null){
							result.setResult(MBeanUtil.makeReturnResultEntityFromList(metadataExtModels,tableName));
