...
public class BookitemController {
	...
	@RequestMapping(value={"importBookitemLocation"})
	@ResponseBody
	public ResultEntity importBookitemLocation(HttpServletRequest request,String req) {
		ResultEntity result = new ResultEntity();
		try {
			result = bookitemService.importBookitemLocation(req);
		} catch (Exception e) {
			ExceptionHelper.afterException(result, Thread.currentThread().getStackTrace()[1].getMethodName(), e);
		}
		return result;
	...
	@RequestMapping(value={"opacQueryBookitem"})
	@ResponseBody
	public ResultEntity opacQueryBookitem(HttpServletRequest request,String req) {
		ResultEntity result = new ResultEntity();
		try {
			result = bookitemService.opacQueryBookitem(req);
		} catch (Exception e) {
			ExceptionHelper.afterException(result, Thread.currentThread().getStackTrace()[1].getMethodName(), e);
		}
		return result;
