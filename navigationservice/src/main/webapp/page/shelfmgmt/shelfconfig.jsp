...
$(function(){
	...
	$("tbody").on("click",".delete",function(){
		shelf_config_id.length = 0;
		shelf_config_id.push( $(this).parent().parent().find("div input[name='idx']").attr("id"));
		$('.word').html("");
	...
	**/
	$(".form-btn.remove").on("click",function(){
		GlobalGLoading();
		var idx = shelf_config_id;
		if(!idx) return;
		var param=new Array();
		for(var i=0;i<idx.length;i++){
			param[i]={
				"shelf_config_id":idx[i],
			};
		}
		$.ajax({
		...
		//获取当前页数
			...
			success:function(data){
				...
				$.each(data.result.rows,function(i,item){
					$("tbody").append(
					"<tr>"+
	            		"<td class='col1'><div class='g-checkbox'><input type='checkbox' name='idx' id='"+item.shelf_config_id+"' /></div></td>"+
