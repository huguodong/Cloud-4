...
$(function(){
	...
	$("tbody").on("click",".delete",function(){
		shelf_group_id.length = 0;
		shelf_group_id.push( $(this).parent().parent().find("div input[name='idx']").attr("id"));
		$('.word').html("");
		...
		$.ajax({
		     ...
		     success:function(data){
				 if(data.state){
					 ...
					 $("#shelf_config_idx").append("<option value=''>请选择</option>");
					 $.each(data.result.rows,function(i,item){
							$("#shelf_config_idx").append(
									"<option value='"+item.shelf_config_id+"'>"+item.shelf_config_id+"</option>"
							);	
						}); 
		...
		$.ajax({
		     ...
		     success:function(data){
				 if(data.state){
					...
					$("#edit_shelf_config_idx").append("<option value=''>请选择</option>");
					 $.each(data.result.rows,function(i,item){
					 		$("#edit_shelf_config_idx").append(
									"<option value='"+item.shelf_config_id+"'>"+item.shelf_config_id+"</option>"
							);	
					 ...
					 $("#edit_shelf_config_idx").val(group[4]);
				 }
		     }
		});
	...
	$("#ineditBtn").on("click",function(){
		...
		var param={
				json1 : {
					...
					"shelf_config_id" : shelf_config_idx,
	...
	**/
	$(".form-btn.remove").on("click",function(){
		GlobalGLoading();
		var idx = shelf_group_id;
		if(!idx) return;
		var param=new Array();
		for(var i=0;i<idx.length;i++){
			param[i]={
				"shelf_group_id":idx[i],
			};
		}
		$.ajax({
			...
			success:function(data){
				...
				$.each(data.result.rows,function(i,item){
					...
					"<tr>"+
						...
						"<td class='col5 hide'>"+item.shelf_config_id+"</td>"+
