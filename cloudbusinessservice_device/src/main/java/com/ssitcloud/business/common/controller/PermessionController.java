...
public class PermessionController {
	 ...
	 * 通过用户idx获取其菜单
	 *
	 * <p>2017年2月15日 下午6:19:46 
	 * <p>create by hjc
	 * @param req
	 * @param json
	 * @return
	 */
	@RequestMapping("/SelMenuByOperIdx")
	@ResponseBody
	public ResultEntity SelMenuByOperIdx(HttpServletRequest req,String json){
		ResultEntity result=new ResultEntity();
		try {
			String reuslt=basicService.SelMenuByOperIdx(json);
			return JsonUtils.fromJson(reuslt, ResultEntity.class);
		} catch (Exception e) {
			String methodName = Thread.currentThread().getStackTrace()[1].getMethodName();
			ExceptionHelper.afterException(result, methodName, e);
	...
	@ResponseBody
	public ResultEntity SelMenuBySsitCloudAdmin(HttpServletRequest request,String json){
		ResultEntity result=new ResultEntity();
		try {
			result=basicService.SelMenuBySsitCloudAdmin(json);
		} catch (Exception e) {
			String methodName = Thread.currentThread().getStackTrace()[1].getMethodName();
			ExceptionHelper.afterException(result, methodName, e);
		}
