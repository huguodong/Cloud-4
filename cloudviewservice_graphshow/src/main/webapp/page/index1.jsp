...
<link href="${pageContext.request.contextPath}/css/skin_1.css" rel="stylesheet" media="screen">
<!--[if lt IE 9]>
		...
		<div class="row">
			<div class="col-md-5">
				<div class="patron-div bg">
					<p>今日到馆人数：<span id="patron-today" class="text-nowrap"><label class="bg2">0</label></span></p>
					<p>当月到馆人数：<span id="patron-all" class="text-nowrap">0</span></p>
				</div>
			...
			<div class="col-md-2">
				<div class="patron-div2">
					<div style="border-bottom: 1px solid #99cccc;height: 50%;">
						<p class="patron-title">图书馆接入总数</p>
						<p><label id="patron-0" style="margin-left: -2px">0</label></p>
					</div>
					<div style="height: 50%;">
						<p class="patron-title">设备接入总数</p>
						<p><label id="patron-1" style="margin-left: -2px">0</label></p>
					</div>
				</div>
			</div>
		</div>
...
$(function(){
	...
	var loadPatronData = function(){
		...
		$.ajax({
			...
			data:{'type':'today,month','library_idx':library_idx},
			dataType: 'json',
			success: function(data){
				if(data.state){
					$(data.result).each(function(i, item){
	...
	//获取图书馆信息
	var loadLibInfo = function(){
		$.ajax({
			type: 'POST',
			url: '${pageContext.request.contextPath}/loadLibInfo',
			...
			success: function(data){
				if(data.state){
					...
					if(result){
						...
						if(libs)$(".patron-div2").find("#patron-0").text(libs);
						if(devices)$(".patron-div2").find("#patron-1").text(devices);
					}
				}
				setTimeout(function(){loadLibInfo()}, 300000);//5分钟
			}
		});
	};
