...
public class SendHearbeatUtil {
	...
	public void sendData(Map<String, String> map) {
		if ("".equals(map.get("req")) || map.get("req") == null) {
			LogUtils.info("---cloudbusinessservice_node节点，心跳检查，请求数据为空，请检查请求参数--------");
		} else {

			String result = hearbeatBusinessService.saveData(map.get("req"));
	...
	public void sendData() {
		...
		if (queue_len > 0) {
			String processInfo = ps == null ? "\"\"" : JSONArray.fromObject(ps)
					.toString();
			String waitInfo = ws == null ? "\"\"" : JSONArray.fromObject(ws)
					.toString();
			String object = "{\"node_name\":\"" + node_name
					+ "\",\"queue_length\":" + queue_len + ",\"processInfo\":"
					+ processInfo + ",\"waitInfo\":" + waitInfo + "}";
	...
	public static List<TimeEntity> average(Map<String, List<TimeEntity>> pm) {
		...
		for (List<TimeEntity> ps : pm.values()) {
			...
			BigDecimal time = new BigDecimal(sum / ps.size()).setScale(0,
