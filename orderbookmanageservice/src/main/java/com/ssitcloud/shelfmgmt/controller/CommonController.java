...
public class CommonController {
	...
	public void exportCountData(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String cdisp = "attachment;filename=\""+new String(request.getParameter("fileName").getBytes("gb2312"),"ISO8859_1")+"-"+dateFormat.format(new Date())+".xls\"";
        response.setHeader("Content-disposition",cdisp);//设置头部信息
        ObjectMapper mapper=new ObjectMapper();
        Map headers = mapper.readValue(request.getParameter("headers"), Map.class);
        String content = request.getParameter("dataset");
        
        List<List> list =mapper.readValue(content, List.class);
        ExportExcelUtils.exportExcel(request.getParameter("title") ,headers, list, response.getOutputStream());
	}
