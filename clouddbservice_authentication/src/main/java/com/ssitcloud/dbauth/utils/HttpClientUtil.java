...
public class HttpClientUtil {
    ...
    static{
		try {
			...
			reqConfigL =RequestConfig.custom().setSocketTimeout(150000).setConnectTimeout(150000).build();
		} catch (IOException e) {
	...
	public static String doPostLongtime(String url,Map<String,String> map,String charset){
		Log.DebugOnScr("DB AUTH doPostLongtime url-->"+url);
		Log.DebugOnScr("DB AUTH doPostLongtime map-->"+map);
		HttpPost httpPost = null;
		String result = null;
		CloseableHttpClient httpClient = null;
		CloseableHttpResponse response=null;
		try{
			...
			httpClient=HttpClients.createDefault();
			response = httpClient.execute(httpPost);
			Log.DebugOnScr("DB AUTH doPostLongtime response-->"+response);
			if(response != null){
				HttpEntity resEntity = response.getEntity();
				Log.DebugOnScr("DB AUTH doPostLongtime resEntity-->"+resEntity);
				if(resEntity != null){
					result = EntityUtils.toString(resEntity,charset);
					Log.DebugOnScr("DB AUTH doPostLongtime result-->"+result);
				}
			}
		}catch(Exception ex){
			//待处理
