...
;(function($){
    ...
    // automatically resize the control
		if( options.resizeContainer ){
			width.container = (Math.max(width.onspan, width.offspan) + width.handle);
		...
		var positionHandle = function (animate){
			...
			if( animate && options.enableFx ){
				$handle.stop().animate({left: x+5}, options.duration, options.easing);
				$onlabel.stop().animate({width: x + 1}, options.duration, options.easing);
		...
		if( options.enableDrag ){
			...
			$(document).bind("mousemove.iButton_" + id + " touchmove.iButton_" + id, function(e) {
				...
				$onlabel.css("width", pct * handleRight + 1);
	...
	var defaults = {
		...
		, classContainerOn:  "ibutton-container_on"
