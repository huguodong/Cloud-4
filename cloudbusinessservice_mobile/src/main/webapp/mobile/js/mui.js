...
var mui = function (a, b) {
    ...
    i.uuid = 0, i.data = {}, i.extend = function () {
        ...
        return i.extend(function () {
            !f || g >= f && i.now() - g > b || f > g && i.now() - f > 8 * b ? d.apply(this, arguments) : (e && e.cancel(), e = i.later(d, b, null, arguments))
        }, {
            stop: function () {
                e && (e.cancel(), e = 0)
            }
        ...
        if (!a) return this;
        if (i.isArrayLike(a)) [].every.call(a, function (a, c) {
...
!function (a, b) {
    function c(c) {
        this.os = {};
        var d = [function () {
            var a = c.match(/(QQBrowser)\/([\d\.]+)/);
            return a && (this.os.qqbrowser = {version: a[2].replace(/_/g, ".")}), !1
        }, function () {
            var a = c.match(/(UCBrowser)\/([\d\.]+)/);
            return a && (this.os.ucbrowser = {version: a[2].replace(/_/g, ".")}), !1
        }, function () {
    ...
    var c, d, e = function (e, f) {
        ...
        switch (e.type) {
            case a.EVENT_END:
                if (!f.isFinal) return;
                for (var i = g.target, j = i, k = !1; j && j !== document; j = j.parentNode) (j.disabled || j.classList && j.classList.contains("mui-disabled")) && (k = !0);
                if (k) return;
    ...
    var c = "mui-slider", d = "mui-slider-group", e = "mui-slider-loop", f = "mui-slider-indicator",
        g = "mui-action-previous", h = "mui-action-next", i = "mui-slider-item", j = "mui-active", k = "." + i,
        l = "." + f, m = ".mui-slider-progress-bar", n = a.Slider = a.Scroll.extend({
            init: function (b, c) {
                this._super(b, a.extend(!0, {
                    fingers: 1,
                    interval: 0,
                ...
                this._reInit(), this.scroller && (this.scrollerStyle = this.scroller.style, this.progressBar = this.wrapper.querySelector(m), this.progressBar && (this.progressBarWidth = this.progressBar.offsetWidth, this.progressBarStyle = this.progressBar.style), this._super(), this._initTimer())
                ...
                if (b.target === c.wrapper) {
                    ...
                    for (var e = c.scroller.querySelectorAll(k), f = [], g = 0, h = e.length; h > g; g++) {
                        var i = e[g];
                        i.parentNode === c.scroller && f.push(i)
                    }
                    var l = d.slideNumber;
                    if (c.loop && (l += 1), !c.wrapper.classList.contains("mui-segmented-control")) for (var g = 0, h = f.length; h > g; g++) {
                        var i = f[g];
                        i.parentNode === c.scroller && (g === l ? i.classList.add(j) : i.classList.remove(j))
                    }
                    ...
                    if (m) {
                        ...
                        if (n.length > 0) for (var g = 0, h = n.length; h > g; g++) n[g].classList[g === d.slideNumber ? "add" : "remove"](j); else {
                            var o = m.querySelector(".mui-number span");
                            if (o) o.innerText = d.slideNumber + 1; else for (var p = m.querySelectorAll(".mui-control-item"), g = 0, h = p.length; h > g; g++) p[g].classList[g === d.slideNumber ? "add" : "remove"](j)
                        }
                    }
                    b.stopPropagation()
                }
    ...
    var u = function (a, b, d) {
        ...
        if (d && e) {
            ...
            var r = q && q.offsetWidth / 2 || 0, s = 0, t = 0, u = 0, w = 0, x = d.classList.contains(g) ? 0 : 5,
                y = "top";
    ...
    var c = "mui-active", d = "mui-tab-item", e = "." + d,
        f = "display:{DISPLAY};position: fixed;left: 0;top: {TOP}px;right: 0;bottom: {BOTTOM}px;width:100%;height:{HEIGHT}px;z-index: 1;border:0;",
        g = function (c, d) {
            this.element = c, this.options = a.extend(!0, {
                type: a.os.plus ? "webview" : "iframe",
                index: 0,
                styles: {top: void 0, bottom: void 0, hardwareAccelerated: !0},
                show: {aniShow: "fade-in", duration: 300}
            }, d || {});
            var f = this.options.styles;
            if ("undefined" == typeof f.top) {
                var g = b.querySelector("header");
                g ? f.top = g.offsetHeight + 1 : f.top = 0
            }
            "undefined" == typeof f.bottom && (f.bottom = c.offsetHeight + 1), this.itemElems = a.slice.call(c.querySelectorAll(e)), this.ids = [], this.shown = [], this.type = this.options.type, this.index = -1, this.init()
        }, h = g.prototype;
    h.init = function () {
        this.itemElems && this.itemElems.length && (a.os.plus && "webview" === this.options.type ? a.plusReady(function () {
            this._initTabByWebview(), this.initEvent(), this.show(this.options.index)
        }.bind(this)) : (this._initTabByIframe(), this.initEvent(), this.show(this.options.index)))
    }, h.initEvent = function () {
        this.element.addEventListener("tap", function (a) {
            for (var c = a.target; c && c !== b; c = c.parentNode) if (c.__mui_tab_id) {
                this.show(c.__mui_tab_id, !1);
                break
            }
        }.bind(this))
    }, h._getId = function (a, b) {
        var c = a.getAttribute("data-tab-id") || b;
        return this.ids.push(c), a.__mui_tab_id = c, c
    }, h._initTabByWebview = function () {
        for (var a = plus.webview.currentWebview(), b = 0, c = this.itemElems.length; c > b; b++) {
            var d = this.itemElems[b], e = d.href, f = this._getId(d, e),
                g = plus.webview.create(e, f, this.options.styles);
            g.hide("none"), a.append(g)
        }
        this.type = "webview"
    }, h._initTabByIframe = function () {
        for (var a = this.options.styles, c = parseFloat(a.top), d = parseFloat(a.bottom), e = parseFloat(window.innerHeight - c - d), g = 0, h = this.itemElems.length; h > g; g++) {
            var i = this.itemElems[g], j = i.href, k = this._getId(i, j), l = b.getElementById(k);
            l || (l = b.createElement("iframe"), l.id = k, l.src = j, l.setAttribute("style", f.replace("{DISPLAY}", "none").replace("{TOP}", c).replace("{BOTTOM}", d).replace("{HEIGHT}", e)), b.body.appendChild(l))
        }
        this.type = "iframe"
    }, h.show = function (a, d) {
        "number" == typeof a && (a = this.ids[a]);
        var e = this.ids.indexOf(a);
        if (~e) {
            if (this.index === e) return !0;
            if (d !== !1) {
                var f = this.element.querySelector("." + c);
                f && f.classList.remove(c);
                var g = this.itemElems[e];
                g.classList.add(c)
            }
            if ("webview" === this.type) ~this.shown.indexOf(a) ? plus.webview.show(a) : (plus.webview.show(a, this.options.show.aniShow, this.options.show.duration), this.shown.push(a)); else if ("iframe" === this.type) {
                var h = b.getElementById(a);
                h.style.display = "block"
            }
            -1 !== this.index && this.hide(this.ids[this.index]), this.index = e
        }
        return !1
    }, h.hide = function (a) {
        if ("number" == typeof a && (a = this.ids[a]), "webview" === this.type) plus.webview.hide(a, "auto"); else if ("iframe" === this.type) {
        ...
        if (1 === this.length) {
            var b = this[0], c = b.mui_plugin_tab;
            return a = a || {}, c || (c = b.mui_plugin_tab = new g(b, a)), c
        }
    }
}(mui, document);